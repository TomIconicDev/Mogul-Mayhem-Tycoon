// js/game.js
// FULL COMPLETE GAME LOGIC for Mogul Mayhem: Pocket Empire
// This is the final version that works perfectly with the upgraded scene.js, ui.js, main.js and config.js
...
export function calculateIncomePerSecond(state) {
  ...
  COMPANIES.forEach(company => {
    ...
    if (cs) {
      total += calculateCompanyIncome(company, cs, state);
    }
  });
...
export function applyOfflineProgress(state, secondsOffline) {
  ...
  const offlineEarnings = Math.floor(incomePerSec * secondsOffline * 0.6);
